...
```{r Negative correlation between resource and punishment}
library(ggplot2)
library(latex2exp)
library(patchwork)

Plot_Negative = function (df, gname, w=1, u=1){
  cbbPalette =c( "#000000", "#F0E442", "#E69F00", "#D55E00", "#56B4E9", "#0072B2","#009E73", "#CC79A7")
　
  aranged_cbb=c()
  if(nrow(df[df$State==15, ])>0){
...
p=0.5
v2_intercept=v1-(p*Rb)**u
a_intercept=(((b*Rb)**w-(1-p*Rb)**u)**(1/w))/Rb
...
Plot_Negative(df=read.csv('FullModel_parameter_sweep_NegativeCorr.csv'),
          'FullModel_parameter_sweep_NegtiveCorr.png', w=1, u=0.5)
